...
//Sound sensor LEDs
int _pinDigitalLeds[] = {A3, A2, A1, 7, 13};
//int _pinDigitalLeds[] = {A7, A6};
...
//Set PIN modes of the PWM pins
  ...
  Serial.print("_pinDigitalLedsCount: "); Serial.println(_pinDigitalLedsCount);
...
void displayCount(){
  ...
  /*
  if(peakToPeak > 5 && (peakToPeak < 22 || peakToPeak > 24)){ //remove noise
    displayPeak = map(peakToPeak,6,30,0,_pinDigitalLedsCount);
  ...
  */
  ...
  if(peakToPeak > 10){ //remove noise
    displayPeak = map(peakToPeak,11,30,0,_pinDigitalLedsCount);
  }
...
void onSoundDetected(int * ledCount){
